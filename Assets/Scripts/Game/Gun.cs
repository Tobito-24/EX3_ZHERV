...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        {
            var bulletAngle = Quaternion.Euler(director.eulerAngles);
            if (shotgunBullets % 2 == 0)
            {
                bulletAngle *= Quaternion.Euler(0,0,-((shotgunBullets / 2 * shotgunSpread)-shotgunSpread/2));
            }
            else
            {
                bulletAngle *= Quaternion.Euler(0,0,(-((shotgunBullets - 1) / 2)*shotgunSpread));
            }

            for (int i = 0; i < shotgunBullets; i++)
            {
                SpawnBullet(director.position, bulletAngle);
                bulletAngle *= Quaternion.Euler(0,0,shotgunSpread); 
            }
