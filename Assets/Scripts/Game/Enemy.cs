...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        {
            rotation = Quaternion.LookRotation((transform.position),Vector3.forward);
        }
        else
        {
            rotation = Quaternion.LookRotation(transform.position-(nearestPlayer.transform.position), Vector3.forward);
            Vector3 newPos = new Vector3();
            Vector3 GetPos = Vector3.Normalize(nearestPlayer.transform.position - transform.position);
            mRigidBody.MovePosition(mRigidBody.position + GetPos * 0.03f);
        }
